...
From a worktree, `scripts/development/preview.sh up` gives dashboard and blog rendering at `https://<slot>-local.blot`, but not template source changes under `app/templates/source` (only the main stack rebuilds those); check those on `https://local.blot`.
