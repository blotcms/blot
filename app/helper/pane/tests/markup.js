...
describe("pane markup", function () {
  ...
  it("emits one child per OS for cells, or only the pinned OS's", function () {
    ...
    expect(all.match(/data-os="(mac|win|linux)"/g).length).toBe(6);
    const pinned = pane.folder("a.md", { files, now: "2026-09-20T15:38:00", os: "linux" }).html;
    expect(pinned.match(/data-os="/g).length).toBe(2);
    expect(pinned).toContain("Today 15:38");
  });
