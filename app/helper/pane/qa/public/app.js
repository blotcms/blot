  ...
  function renderMetrics(r) {
    ...
    const table = h("table", {},
      h("tbody", {},
        ...
        ...r.regions.map((g) => row(`  ${g.name} (${g.kind})`, `${num(g.percent)}% · ink ${num(g.inkPercent, 0)}% · blur ${num(g.blurMae, 1)}% · ΔE ${num(g.flat.deltaE, 1)}`, failed(g.name))),
        ...r.regions.filter((g) => g.flat.hint).map((g) => row("  colour", g.flat.hint)),
        row("Blurred error", num(r.blurMae, 1) + "%"),
