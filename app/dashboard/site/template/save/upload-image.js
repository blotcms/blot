...
const templates = (id) => new Promise((resolve, reject) => Template.getTemplateList(id, (e, list) => e ? reject(e) : resolve(list || [])));
...
async function removeAssetsIfUnreferenced(req, old) {
  ...
  let list;
  try { list = await templates(req.blog.id); }
  catch (error) {
  ...
  const referenced = list.some((template) => template && Object.values(template.locals || {}).some((value) => value && value.url === old.url));
